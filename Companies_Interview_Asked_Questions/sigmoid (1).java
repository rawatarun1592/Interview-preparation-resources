...
public static ListNode subtractTwoNumbers(ListNode l1, ListNode l2) {
    ...
    ListNode dummyHead = new ListNode(0);
    ListNode current = dummyHead; // Pointer to the current node in result list
    int borrow = 0; // Variable to store the borrow during subtraction
    ...
    while (l1 != null || l2 != null) {
        ...
        // Calculate difference of current digits after considering borrow
        int diff = x - y - borrow;
        if (diff < 0) {
            diff += 10; // If difference is negative, borrow from the next higher digit
            borrow = 1;
        } else {
            borrow = 0;
        }
