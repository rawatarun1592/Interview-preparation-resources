...
// Definition of the Node class representing a binary tree node
class Node {
    int val;        // Value of the node
...
// Main class for Tree Zigzag Traversal
class TUF {
    ...
    public static ArrayList<ArrayList<Integer>> zigzagLevelOrder(Node root) {
        ...
        // ArrayList to store the result of zigzag level order traversal
        ArrayList<ArrayList<Integer>> wrapList = new ArrayList<>();

        // Check if the root is null, return an empty wrapList if true
        if (root == null) return wrapList;

        // Enqueue the root to start the traversal
        queue.offer(root);
        // Variable to toggle the direction of traversal (left to right or right to left)
        boolean flag = true;
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < levelNum; i++) {
                ...
                // Depending on the flag, add the value of the current node to the subList
